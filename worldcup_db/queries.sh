...
# WINNER_ID=$($PSQL "select winner_id from games where round='Final' and year=2018")
...
echo "$($PSQL "select name from teams left join games on (games.winner_id=teams.team_id or games.opponent_id=teams.team_id) where year=2014 and round='Eighth-Final' group by name")"
